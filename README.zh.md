...
- [Rust](https://www.rust-lang.org)（通过 flutter_rust_bridge，高性能逻辑）
...
1. **情绪调色盘系统**：12色阶 + 预设情绪 + AI 推荐三级体验
2. **Rust 渐进式初始化**：不阻塞启动 + 保存时智能降级处理
3. **API 灵活切换**：火山方舟普通版/Coding Plan 支持
4. **分析统计优化**：同一天多篇日记取平均值，折线图 + 完整数据用于 AI 分析
5. **错误处理增强**：API 调用可选不显示错误提示
