...
class HttpUtil {
  ...
  Dio get dioNoErrorToast {
    if (_dioNoErrorToast == null) {
      _dioNoErrorToast = Dio(
        BaseOptions(connectTimeout: const Duration(seconds: 5)),
      );
      _dioNoErrorToast?.interceptors.add(
        InterceptorsWrapper(
          onRequest: (options, handler) {
            if (_enableLogging) {
              logger.i('Request (no toast): ${options.method} ${options.path}');
            }
            handler.next(options);
          },
          onResponse: (response, handler) {
            if (_enableLogging) {
              logger.i('Response (no toast): ${response.statusCode}');
            }
            handler.next(response);
          },
        ),
  ...
  Future<Stream<String>?> postStreamNoErrorToast(
    String path, {
    Map<String, dynamic>? header,
    Object? data,
  }) async {
    final Response<ResponseBody> response = await dioNoErrorToast.post(
      path,
      options: Options(responseType: ResponseType.stream, headers: header),
      data: data,
    );
